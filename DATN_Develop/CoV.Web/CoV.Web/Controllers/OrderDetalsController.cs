...
namespace CoV.Web.Controllers
{
    public class OrderDetalsController : Controller
    {
        ...
        {
            var orderdetails = _orderDetailsService.GetAll().Where(c=>c.StatusId ==1 || c.StatusId == 5 || c.StatusId == 1002).ToList();
            return 
            View(orderdetails);
        }
        ...
        {
            var orderdetails = _orderDetailsService.GetAll().Where(c=>c.StatusId == 2 || c.StatusId == 1003).ToList();
            return 
                View(orderdetails);
        }
        ...
        public IActionResult CartCustomer(int id)
        {
            ...
            var orderDetails = _orderDetailsService.GetAll().Where(x => x.PhoneNumber.Equals(sessionPhone) && x.StatusId != 2 &&  x.StatusId !=3 && x.StatusId != 1003); // danh sach don hang cua khach hang
        ...
        {
            _orderDetailsService.Delete1(id);
            return Redirect("/OrderDetals/CartCustomer");
        }

        public IActionResult CancelOrder(int id)
        {
            _orderDetailsService.CancelOrder(id);
            return Redirect("/OrderDetals/Index");
        }

        public IActionResult CancelOrderDelete(int id)
        {
            _orderDetailsService.CancelOrderDelete(id);
            return Redirect("/OrderDetals/Index");
        }
