...
namespace CoV.Service.Service
{
    public interface IProductService
    {
        ...
        /// </summary>
        /// <returns></returns>
        IEnumerable<ProductViewModel> GetAll1(string search);
        ...
        ProductViewModel GetByProductId(int id);
        ProductViewModel GetByIdCart(int id);
        ProductViewModel GetByIdSku(string sku);
    ...
    public class ProductService :IProductService
    {
        ...
        public ProductViewModel GetById(int id)
        {
            ...
            if (id <= 0)
            {
                product.AvatarDetails = "img-product-03.jpg";
                return product;
            }
            else
            {
        ...
        public  ProductViewModel GetByProductId(int id)
        {
            var productNew = _unitOfWork.ProductRespository.ObjectContext.AsNoTracking().FirstOrDefault(x=>x.Id.Equals(id));
        ...
        public ProductViewModel GetByIdSku(string sku)
        {
            var productNew = _unitOfWork.ProductRespository.ObjectContext.AsNoTracking()
                .FirstOrDefault(x => x.Sku.Equals(sku));
            return _mapper.Map<ProductViewModel>(productNew);
        }
...
//                product.PhotoPath = model.photoPath;
        ...
        IEnumerable<ProductViewModel> IProductService.GetAll1(string search)
        {
            var products = _unitOfWork.ProductRespository.ObjectContext.Where(s => s.Name.Contains(search)).ToList();
               // .Include(x => x.Gender)
               // .Include(x => x.CategoryProduct)
               // .Include(x => x.MakerProduct)
               //.ToList();//Entity 
