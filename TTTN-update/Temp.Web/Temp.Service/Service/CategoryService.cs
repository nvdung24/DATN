...
namespace Temp.Service.Service
{
    ...
    public class CategoryService : ICategoryService
    {
        ...
        public bool CheckExist(CategoryDto categoryDto)
        {
            var number = _unitofWork.CategoryBaseService.ObjectContext.Where(s => s.Name.ToUpper().Equals(categoryDto.Name.ToUpper())).Count();
            if (number <= 0)
                return true;
            else
                return false;
